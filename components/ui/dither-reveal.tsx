...
function settings(p: DitherRevealProps) {
  ...
  return {
    ...
    brightness: clamp(p?.brightness, 10, 300, DEFAULTS.brightness) / 100,
    contrast: clamp(p?.contrast, 10, 300, DEFAULTS.contrast) / 100,
...
export default function DitherReveal(props: DitherRevealProps) {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      gl!.uniform1f(uBrightness, L.brightness);
      gl!.uniform1f(uContrast, L.contrast);
...
void main() {
    ...
    gray = clamp((gray * uBrightness - 0.5) * uContrast + 0.5, 0.0, 1.0);
